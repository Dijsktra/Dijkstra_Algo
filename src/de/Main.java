...
public class Main {
	...
	public static void main(String[] args) {
		...
		a.adjacencies = new Kante[] { new Kante(b, 4), new Kante(c, 2),
				new Kante(d, 5) };
		b.adjacencies = new Kante[] { new Kante(a, 4), new Kante(e, 6),
				new Kante(f, 4) };
		c.adjacencies = new Kante[] { new Kante(a, 2), new Kante(d, 5) };
		d.adjacencies = new Kante[] { new Kante(a, 5), new Kante(e, 5),
				new Kante(f, 3) };
		e.adjacencies = new Kante[] { new Kante(b, 6), new Kante(d, 5),
				new Kante(f, 1) };
		...
		do {
			...
			// ansonsten f�ge aktiv zu besucht hinzu
			else {

				besucht.add(aktiv);
			}

			// betrachte jeden von a aus direkt erreichbaren nachbarknoten n
			// (aktiv.adjacencies[i])
			for (int i = 0; i < aktiv.adjacencies.length; i++) {
				...
				if (rand.contains(aktiv.adjacencies[i].knoten)) {
					...
					if (gewicht < aktiv.adjacencies[i].knoten.gewicht
							|| aktiv.adjacencies[i].knoten.gewicht == 0) {
